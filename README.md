...
- Both `publish` and `enabled` should **not** mutate the results (particularly the `controlResult`). Paleontologist does not do any deep cloning on its own. If you want to alter the results for a custom compare, consider using something like [Lodash's](https://lodash.com/) `cloneDeep` first. ([lodash_require](https://github.com/TokenChingy/lodash_require) might help; I haven't tried it though.)
